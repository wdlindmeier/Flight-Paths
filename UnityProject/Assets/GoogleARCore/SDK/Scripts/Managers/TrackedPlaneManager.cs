    ...
    public class TrackedPlaneManager
    {
        ...
        public void OnApplicationPause(bool isPaused)
        {
            ...
            for (int i = 0; i < m_allPlaneRecords.Count; i++)
            {
                ...
                invalidPlane.timestamp = m_allPlaneRecords[i].m_planeData.timestamp + 1;
                m_allPlaneRecords[i].m_updatePlane(invalidPlane, null);
            }

            m_allPlaneRecords.Clear();
            m_newPlaneRecords.Clear();
        }

        private void _UpdateFrameIfNeeded()
        {
            ...
            else if ((Time.time - m_lastUpdateTime) < MIN_DELTA_BETWEEN_UPDATES)
            {
                m_newPlaneRecords.Clear();
                return;
            }
            ...
            _UpdateMostRecentApiPlanes();
            _UpdatePlanes(m_mostRecentApiPlanes, m_newPlaneRecords, m_allPlaneRecords);

            return;
        }

        private void _UpdateMostRecentApiPlanes()
        {
            ...
            {
                ARDebug.LogErrorFormat("Failed to get planes from the ARCore API.");
                return;
            }
        ...
        private void _UpdatePlanes(List<ApiPlaneData> latestPlaneData, List<TrackedPlaneRecord> newPlaneRecords,
            List<TrackedPlaneRecord> planeRecords)
        {
            ...
            for (int i = 0; i < latestPlaneData.Count; i++)
            {
                ...
                if (planeRecordIndex != -1)
                {
                    ...
                    planeRecords[planeRecordIndex].m_updatePlane(latestPlaneData[i], null);
                }
                else
                {
                    Action<ApiPlaneData, TrackedPlane> updatePlane;
            ...
            for (int i = planeRecords.Count - 1; i >= 0; i--)
            {
                if (!m_tmpLastestPlanesHash.Contains(planeRecords[i].m_planeData.id))
                {
                    ...
                    invalidPlane.timestamp = planeRecords[i].m_planeData.timestamp + 1;
                    planeRecords[i].m_updatePlane(invalidPlane, null);
        ...
        private struct TrackedPlaneRecord
        {
            ...
            public Action<ApiPlaneData, TrackedPlane> m_updatePlane;
